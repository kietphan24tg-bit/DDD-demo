...
import type { AggregateRoot } from '../../../domain/entities/aggregate';
import type { EmitDomainEvents } from '../../../domain/events/domain-event.type';
import type {
    Paginated,
...
> implements RepositoryPort<Aggregate> {
    protected abstract tableName: keyof DbTables & string;
    ...
    async insert(entity: Aggregate) {
        ...
        const result = await this.db
            ...
            // @ts-expect-error generic DbModel cannot be narrowed to InsertExpression for a specific table
            .values(record)
    ...
    updateBulkMany(
        entities: Aggregate[]
    ): Promise<void> | Promise<Aggregate[]> {
        throw new Error('Method not implemented.');
    }
